...
<body>
<div class="wrap1">
<div class="hdr1">Great Plains Member Credit Union &mdash; Back Office</div>
<div class="pnl1">
${body}
</div>
</div>
...
export function renderHome(username: string): string {
  return layout(
    "Member Lookup",
    `
...
<table class="grid1">
  ...
  <tr>
    ...
    <td>
      <table class="grid1">
        <tr><th>Current Savings Balance</th><td>$${member.savingsBalance.toFixed(2)}</td></tr>
      </table>
    </td>
  </tr>
</table>
<form method="get" action="/member/${encodeURIComponent(member.id)}/sub-account">
...
<p><a href="/">Return to Member Lookup</a></p>`,
  );
}

export function renderPermissionDenied(id: string): string {
  return layout(
    "Access Denied",
    `
...
export function renderTransientInterstitial(retryPath: string): string {
  return layout(
    "Please Wait",
    `
...
export function renderSubAccountForm(memberId: string): string {
  return layout(
    "Open Sub-Account",
    `
<h3>Open Sub-Account &mdash; Member ${escapeHtml(memberId)}</h3>
<form method="post" action="/member/${encodeURIComponent(memberId)}/sub-account">
  <table class="grid1">
    <tr>
...
export function renderUnexpectedDialog(memberId: string): string {
  return layout(
    "Confirm",
    `
<div class="msg1">Are you sure you want to proceed to Open Sub-Account for Member
${escapeHtml(memberId)}?</div>
<p><a href="/member/${encodeURIComponent(memberId)}/sub-account?ack=1">Proceed</a></p>`,
  );
}

export function renderSubAccountConfirm(memberId: string, draft: SubAccountDraft): string {
  return layout(
    "Confirm Sub-Account",
    `
...
<form method="post" action="/member/${encodeURIComponent(memberId)}/sub-account/confirm">
  <p><button type="submit">Confirm</button></p>
</form>`,
  );
}

export function renderCreated(memberId: string): string {
  return layout(
    "Sub-Account Created",
    `
<div class="msg1 ok">Created</div>
<p><a href="/member/${encodeURIComponent(memberId)}">Return to Member ${escapeHtml(memberId)}</a></p>`,
  );
}

export function renderNoPendingSubAccount(memberId: string): string {
  return layout(
    "Confirm Sub-Account",
    `
<div class="msg1 err">No pending sub-account request for Member ${escapeHtml(memberId)}.</div>
<p><a href="/member/${encodeURIComponent(memberId)}/sub-account">Start Open Sub-Account</a></p>`,
  );
}
